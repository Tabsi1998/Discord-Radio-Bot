...
Voice guard can now be controlled globally through env defaults and per guild through the dashboard or `/voiceguard`. The protection itself is active on all plans so active OmniFM sessions do not silently accept foreign channel moves. The per-guild layer overrides the move policy, while repeated foreign moves are still governed by the documented global guard thresholds.
